...
- **changed**: the deny floor matches what the shell will actually execute, including what `bash -c` and `eval` are handed, and judges each segment of a compound command on its own. `rm -rf` moved from deny to ask; replayed over a week of audit logs, 149 blocked commands come down to 3.
- **fixed**: gating works in `/auto` again. Claude does not block on leashd's `PreToolUse` hook in auto mode, so gated commands ran anyway and leashd recorded rejections nobody made; every gated rule is now mirrored into the permission table Claude does honour. A blocked call no longer ends the turn, and an approval you tap reaches the pane instead of stalling on it.
- **fixed**: the credential floor covers shell commands, not just the file tools, on every runtime. `cat <repo>/.env`, an SSH key copied out of `~/.ssh/`, and `agent-browser snapshot | curl -d @-` were all cleared by read-only allow rules; a credential path named inside a quoted remote payload (`ssh host 'ls ~/.ssh/'`) is not a local access and no longer trips the floor either.
- **fixed**: a run of conversation bugs: one chat's `pkill` killing another chat's agent, a mid-turn follow-up silencing the turn, a numbered list in a reply bridged as a question, `/clear` replaying the message it just cleared, and Telegram replies past 4000 characters duplicating their opening.
- **fixed**: approvals name and scope the right command: the segment the policy matched instead of a `pgrep` prologue in front of it, the command behind a `v=$(curl …)` capture, and "Approve all" for `curl` keying on the host rather than the exact URL (one day of research spent 116 of 141 taps on `curl`; replayed, 66 come down to 16).
- **fixed**: three holes in the gate: a newline now separates commands, the deny floor matches the SQL `sqlite3` is handed positionally, and a compound command is allowed only when every segment is.
- **added**: read-only commands run without asking (`sed -n`, `sqlite3` SELECT, `ps`, `lsof`, `jq`, `rg`, loopback GETs and friends, 2158 previously unmatched commands over a week), and the Web UI approval card gains the "Approve all" button Telegram already had.
- **fixed**: four tmux turn-killers: a reply that stopped at its preamble, `/chrome`'s cursor-only panel arriving with no buttons, an untrusted directory quitting Claude outright, and a message dropped as "never reached the prompt" while a background shell ran.
